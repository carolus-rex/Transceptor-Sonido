...
def frame_length(header):
    bitrate = BITRATE_TABLE[header[2] >> 4]
    sample_rate = SAMPLERATE_TABLE[(header[2] & 0b00001100) >> 2]
    padding = (header[2] & 0b00000010) >> 1
...
class Lame(threading.Thread):
    ...
    def start(self, *args, **kwargs):
        ...
        if self.channels == 1:
            ...
            call.append("m")
        call.append("-s")
        call.append(str(self.samplerate / 1000))
        call.append('-r')
...
def open(name, mode):
    return FileLikeLame(name, mode)


class FileLikeLame(Lame):
    ...
    def start(self, *args, **kwargs):
        self.started = True
        super(FileLikeLame, self).start(*args, **kwargs)

    def setframerate(self, rate):
        self.samplerate = rate
        self._recalcular_chunk_size()
    ...
    def setsampwidth(self, width):
        self.input_wordlength = width
        self._recalcular_chunk_size()

    def _bytes_to_nparray(self, data):
        return numpy.frombuffer(data, dtype=numpy.int16).reshape((-1, 2))

    def writeframes(self, data):
        if not self.started:
            self.start()
        if isinstance(data, bytes):
            data = self._bytes_to_nparray(data)
        self.add_pcm(data)

    def getduration(self):
        try:
            audio = MP3(self.ofile.name)
